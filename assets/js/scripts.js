...
function SelectionChangedDivisionBoosting() {
  ...
  if (desiredElo === "master") {
    document.getElementsByClassName('eloicon').style.visibility='hidden';
  } else {
    $("#divisions").show();
  }
...
function SelectionChangedWinBoosting(){
  ...
  if($("#priorityOrder_winboost").is(':checked')){
    ...
    streaming = "false";
  if($("#streamingOrder_winboost").is(':checked')){
    streaming = "true";
  }
...
function  SelectionChangedPlacementBoosting(){
  ...
  streaming = "false";
  if($("#streamingOrder_placementboost").is(':checked')){
    streaming = "true";
  }
...
function checkout(){
  ...
  var url = "backend/checkout.php?type="+type+"&from_to_str="+fromToStr+"&priority="+priority+"&streaming="+streaming+"&price="+price+"&currency="+getCurrency()+"&server="+server+"&acc_info="+accInfo;
